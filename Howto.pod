...
method and load the C<Games::Lacuna::Task::Role::PlanetRun> role, otherwise 
...
Optionall you can set the 'NoAutomatic' trait, indication that this task
...
Tasks can also be called/embedded/executed directly
 ...
 my $task = Games::Lacuna::Task::Action::Archaeology->new(
    database    => $path_to_config_dir, # only needed if client is not provided and if different from default
...
If you have checked out this distribution directly from github then you will 
probably need to install L<Module::Install> before running the makefile.
